...
const DrawBtn = styled.button`
    ...
    box-shadow:1px 1px 20px 16px ${props =>
      props.isClicking ? '#cc6140' : '#29b6e4'} inset;
    border-radius:5px;
    cursor:pointer;
    padding:0;
    font-size:45px;
...
function GetDrawBtn({ isClicking, onClick }) {
  return (
    <DrawBtn isClicking={isClicking} onClick={onClick}>
      {isClicking ? '抽奖中...' : '开始'}
    </DrawBtn>
  )
}
...
class Draw extends PureComponent {
  ...
  render() {
    ...
    const getIsActive = item => (item.id === activeIndex ? 1 : 0)
    return (
            ...
            <GiftBox>
              {giftList.map((gift, index) => (
                <GiftItem
                  ...
                  key={index}
                >
                  <GiftImg iconSrc={gift.icon} />
                  <div className="gift-name">
                    {gift.name}X{gift.count}
                  </div>
                </GiftItem>
              ))}
